...
install:
	...
	until kubectl get crds pgbouncerversions.catalog.kubedb.com -o=jsonpath='{.items[0].metadata.name}' &> /dev/null; do sleep 1; done; \
	kubectl wait --for=condition=NamesAccepted crds -l app.kubernetes.io/name=kubedb --timeout=5m; \
	kubectl get crds pgbouncerversions.catalog.kubedb.com -o yaml
	cd ../installer; \
